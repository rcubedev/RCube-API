...
scoreboard players remove $loop.runs_needed rcubeAPI_centreString 1

# Loop until meets requirement for times run
execute unless score $loop.runs_needed rcubeAPI_centreString matches ..0 run function rcube:api/centre_string/zprivate/append