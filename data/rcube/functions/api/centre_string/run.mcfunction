...
# Clear stale data
data remove storage rcube:api/centre_string temp

# Prevent loop
data modify storage rcube:api/centre_string temp.string set string storage rcube:api/centre_string input.string
...
execute if data storage rcube:api/centre_string {temp:{bold:true}} run scoreboard players operation $px rcube_api.centre_string += $char_count rcube_api.centre_string
...
# Clear output array
data modify storage rcube:api/centre_string output set value [{array:'""'}]
