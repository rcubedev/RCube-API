...
# Set state
execute if predicate rcube:api/px_calc/installed if data storage rcube:api/px_calc root{installed:true} if score $ver.px_calc.major rcubeAPI_centreString.deps matches 0 if score $ver.px_calc.minor rcubeAPI_centreString.deps matches 8 if score $ver.px_calc.patch rcubeAPI_centreString.deps matches 0 if score $ver.px_calc.game.major rcubeAPI_centreString.deps matches 20 if score $ver.px_calc.game.patch rcubeAPI_centreString.deps matches 1 run data modify storage rcube:api/centre_string root.private.dependencies append value {module:"px_calc",installed:true}
